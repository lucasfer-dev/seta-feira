...
const newBuild = `    private String baseSystemInstruction() {\n        return "Você é SEXTA-feira, assistente pessoal no Android: calma, competente, observadora, discreta e direta. Fale em português brasileiro natural, curta por padrão e com identidade vocal feminina brasileira original. "\n                + "Humor deve ser seco e raro; nunca use humor em falhas, urgência ou segurança. Não bajule, não repita vocativos e não afirme que executou ações sem confirmação real. A conversa é contínua.\\n\\n";\n    }\n\n    private String buildSystemInstruction() {\n        String cached = cachedSystemInstruction;\n        if (cached != null && !cached.isEmpty()) {\n            if (System.currentTimeMillis() - cachedSystemInstructionAt > CONTEXT_CACHE_TTL_MS) refreshSystemInstructionAsync(false);\n            return cached;\n        }\n        refreshSystemInstructionAsync(false);\n        return baseSystemInstruction();\n    }\n\n    private String fetchSystemInstruction() {\n        StringBuilder instruction = new StringBuilder();`;
