...
test('voice sync is scoped, TTL-aware and stale-while-revalidate', () => {
  ...
  assert.match(source, /memoryVisible/); assert.match(source, /cacheKey/); assert.match(source, /deviceId/); assert.match(source, /expiresAt/); assert.match(source, /STALE-WHILE-REVALIDATE/);
});
