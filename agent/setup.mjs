...
function ask(label, fallback = '') {
  return rl.question(`${label}${fallback ? ` [${fallback}]` : ''}: `).then(value => String(value || '').trim() || fallback);
}
...
async function main() {
  ...
  while (true) {
    ...
    if (!fs.existsSync(resolved)) {
      console.log('  Pasta não encontrada; tente de novo.');
      continue;
    }
    const defaultName = path.basename(resolved).toLowerCase().replace(/[^a-z0-9_-]/g, '-');
    const name = await ask('Nome curto desse projeto', defaultName);
    projects[name] = resolved;
  }

  const codeCommand = where('code') || 'code';
  const codexCommand = where('codex') || 'codex';
  const browser = browserPath();
  const profileDir = path.join(os.homedir(), '.sexta-browser-profile');
  const config = {
    deviceName,
    apps: {
      vscode: { command: codeCommand, args: [] },
      browser: browser ? { command: browser, args: [] } : { command: 'cmd', args: ['/c', 'start', '', 'https://www.google.com'] }
    },
    projects,
  ...
  writeEnv({
    SEXTA_BASE_URL: baseUrl.replace(/\/$/, ''),
    SEXTA_AGENT_TOKEN: pair.token,
    SEXTA_DEVICE_ID: deviceId
  });

  console.log(`\n✓ PC pareado como ${deviceName}`);
  console.log(`✓ config.json criado com ${Object.keys(projects).length} projeto(s)`);
  console.log('✓ token específico deste dispositivo salvo em .env.local\n');
