...
function Get-SextaWindows {
  ...
  [SextaWindowNative]::EnumWindows({ param($h,$lp)
    ...
    $title=$sb.ToString().Trim(); if([string]::IsNullOrWhiteSpace($title)){return $true}
    $pid=[uint32]0; [void][SextaWindowNative]::GetWindowThreadProcessId($h,[ref]$pid)
...
function Find-SextaWindow([string]$needle,[long]$preferredHwnd=0){
  ...
  if($preferredHwnd -ne 0){$exact=$all|Where-Object{$_.hwnd -eq $preferredHwnd}|Select-Object -First 1;if($exact){return $exact}}
  $n=$needle.Trim()
...
function Focus-SextaHwnd([IntPtr]$h){
  ...
  try{
    ...
    [void][SextaWindowNative]::BringWindowToTop($h);[void][SextaWindowNative]::SetActiveWindow($h);[void][SextaWindowNative]::SetForegroundWindow($h)
  }finally{if($attached){[void][SextaWindowNative]::AttachThreadInput($fgThread,$targetThread,$false)}}
  for($i=0;$i-lt 15;$i++){if([SextaWindowNative]::GetForegroundWindow()-eq$h){return $wasMin};Start-Sleep -Milliseconds 70;[void][SextaWindowNative]::BringWindowToTop($h);[void][SextaWindowNative]::SetForegroundWindow($h)}
  try{$pid=[uint32]0;[void][SextaWindowNative]::GetWindowThreadProcessId($h,[ref]$pid);$ws=New-Object -ComObject WScript.Shell;[void]$ws.AppActivate([int]$pid)}catch{}
...
export async function listWindows(limit = 30) {
  ...
  const script = `$all=@(Get-SextaWindows|Select-Object -First ${max});[pscustomobject]@{windows=$all;count=$all.Count;active=$all|Where-Object{$_.active}|Select-Object -First 1}|ConvertTo-Json -Depth 5 -Compress`;
...
$h=[IntPtr][long]$w.hwnd;$before=[SextaWindowNative]::GetForegroundWindow().ToInt64();$restored=Focus-SextaHwnd $h
$after=[SextaWindowNative]::GetForegroundWindow().ToInt64();$r=New-Object SextaWindowNative+RECT;[void][SextaWindowNative]::GetWindowRect($h,[ref]$r)
...
export async function setWindowState(title, state, hwnd = 0) {
  ...
  const handle = Number(hwnd) || 0;
  const script = String.raw`
$w=Find-SextaWindow (Get-SextaUtf8 'SEXTA_TITLE') ${handle};$h=[IntPtr][long]$w.hwnd
[void][SextaWindowNative]::ShowWindowAsync($h,${cmd});Start-Sleep -Milliseconds 180
if('${desired}' -eq 'restore'){[void](Focus-SextaHwnd $h)}
$min=[SextaWindowNative]::IsIconic($h);$max=[SextaWindowNative]::IsZoomed($h)
$verified=if('${desired}' -eq 'minimize'){$min}elseif('${desired}' -eq 'maximize'){$max}else{(-not $min)}
...
[void][SextaWindowNative]::SetWindowPos($h,[IntPtr]::Zero,${px},${py},${pw},${ph},0x0040);Start-Sleep -Milliseconds 160
$r=New-Object SextaWindowNative+RECT;[void][SextaWindowNative]::GetWindowRect($h,[ref]$r)
$verified=([Math]::Abs($r.Left-${px})-le 3 -and [Math]::Abs($r.Top-${py})-le 3 -and [Math]::Abs(($r.Right-$r.Left)-${pw})-le 6 -and [Math]::Abs(($r.Bottom-$r.Top)-${ph})-le 6)
...
export async function closeWindowNative(title, hwnd = 0) {
  ...
  const handle = Number(hwnd) || 0;
  const script = String.raw`
$w=Find-SextaWindow (Get-SextaUtf8 'SEXTA_TITLE') ${handle};$h=[IntPtr][long]$w.hwnd;$title=$w.title;$pid=$w.pid;$proc=$w.process
[void][SextaWindowNative]::PostMessage($h,0x0010,[IntPtr]::Zero,[IntPtr]::Zero)
$closed=$false;for($i=0;$i-lt 30;$i++){Start-Sleep -Milliseconds 100;if(-not [SextaWindowNative]::IsWindow($h)){$closed=$true;break}}
