...
function semanticText(value = '') { return String(value || '').replace(/([a-z0-9])([A-Z])/g, '$1 $2').replace(/[_-]+/g, ' ').trim(); }
...
function run(script, timeout=12000, data={}){
 ...
 return new Promise((resolve,reject)=>{
   ...
   const timer=setTimeout(()=>{try{child.kill()}catch{};finish(reject,new Error('PC_UI_ACTION_TIMEOUT'))},timeout);
...
export async function uiAction({action='invoke',name='',automationId='',controlType='',value=''}={}){
 ...
 if(['set_value','type'].includes(op)&&(isCredential(n)||isCredential(id)))throw new Error('PC_UI_PASSWORD_FIELD_BLOCKED');
...
switch($op){
 ...
 'set_value' {
   try{$p=$node.GetCurrentPattern([System.Windows.Automation.ValuePatternIdentifiers]::Pattern);if($p -and -not $p.Current.IsReadOnly){$p.SetValue($value);$done=$true;$via='ValuePattern';Start-Sleep -Milliseconds 70;$verified=([string]$p.Current.Value -eq $value)}}catch{}
   if(-not $done){try{$p=$node.GetCurrentPattern([System.Windows.Automation.LegacyIAccessiblePatternIdentifiers]::Pattern);if($p){$p.SetValue($value);$done=$true;$via='LegacyValue';Start-Sleep -Milliseconds 70;$verified=([string]$p.Current.Value -eq $value)}}catch{}}
   if(-not $done){$node.SetFocus();Start-Sleep -Milliseconds 80;[System.Windows.Forms.SendKeys]::SendWait('^a');$done=[SextaUiNative]::Unicode($value);$via='keyboard-fallback';$verified=$done}
 }
 'type' {
