...
function setSelectedTarget(port, id) {
  const targetId = String(id || '').trim(); selectedTargetId = targetId || null;
  if (!targetId) return; try { fs.writeFileSync(selectionPath(port), targetId, 'utf8'); } catch {}
}
...
function resolveBrowserExecutable(cfg = {}) {
  for (const candidate of executableCandidates(cfg)) {
    if (/[/\\]/.test(candidate)) { if (fs.existsSync(candidate)) return candidate; } else return candidate;
  }
  throw new Error('PC_BROWSER_NOT_FOUND');
}
async function debugReady(port) {
  try { const response = await fetch(`http://127.0.0.1:${port}/json/version`, { signal: AbortSignal.timeout(900) }); return response.ok; } catch { return false; }
}
async function ensureBrowser(cfg = {}) {
  ...
  for (let i=0;i<30;i+=1){ if(await debugReady(port)) return port; await sleep(180); }
  throw new Error('PC_BROWSER_DEBUG_NOT_READY');
}
async function pageTargets(port) {
  const response=await fetch(`http://127.0.0.1:${port}/json/list`,{signal:AbortSignal.timeout(1500)}); if(!response.ok) throw new Error(`PC_BROWSER_TARGETS_${response.status}`);
  const targets=await response.json(); return (Array.isArray(targets)?targets:[]).filter(item=>item?.type==='page'&&item?.webSocketDebuggerUrl&&!String(item.url||'').startsWith('devtools://'));
}
...
async function pageTarget(port) {
  ...
  const persisted=selectedTargetId||readPersistedSelection(port); const selected=persisted?pages.find(item=>item.id===persisted):null;
  // Once a tab was selected, keep that exact target even while its URL is temporarily about:blank during navigation.
  const target=selected||pages.find(usefulPage)||pages[0]; setSelectedTarget(port,target.id); return target;
}
async function activateTarget(port,id) {
  const targetId=String(id||''); if(!targetId) throw new Error('PC_BROWSER_TAB_ID_REQUIRED');
  const response=await fetch(`http://127.0.0.1:${port}/json/activate/${encodeURIComponent(targetId)}`,{signal:AbortSignal.timeout(1500)}); if(!response.ok) throw new Error(`PC_BROWSER_TAB_ACTIVATE_${response.status}`); setSelectedTarget(port,targetId); return true;
}
async function cdpCall(port,method,params={}) {
  const target=await pageTarget(port); const Ws=globalThis.WebSocket; if(!Ws) throw new Error('PC_BROWSER_WEBSOCKET_UNAVAILABLE');
  return new Promise((resolve,reject)=>{ const ws=new Ws(target.webSocketDebuggerUrl); const id=Math.floor(Math.random()*1_000_000)+1;
    const timer=setTimeout(()=>{try{ws.close();}catch{};reject(new Error(`PC_BROWSER_CDP_TIMEOUT:${method}`));},7000);
    ws.addEventListener('open',()=>ws.send(JSON.stringify({id,method,params}))); ws.addEventListener('error',()=>{clearTimeout(timer);reject(new Error('PC_BROWSER_CDP_SOCKET_FAILED'));});
...
async function evaluate(port,expression,returnByValue=true){return cdpCall(port,'Runtime.evaluate',{expression,awaitPromise:true,returnByValue,userGesture:true});}
async function browserState(port){const raw=await evaluate(port,`(() => JSON.stringify({url:location.href,title:document.title,readyState:document.readyState,timeOrigin:performance.timeOrigin||0,historyLength:history.length,textMarker:String(document.body?.innerText||'').replace(/\\s+/g,' ').trim().slice(0,1800),active:(()=>{const e=document.activeElement;return e?[e.tagName,e.id,e.getAttribute('name'),e.getAttribute('aria-label')].filter(Boolean).join('|'):'';})()}))())`);return parseEval(raw)||{};}
...
async function waitForStateChange(port,before,timeoutMs=2600){const deadline=Date.now()+timeoutMs;let after=before;while(Date.now()<deadline){try{after=await browserState(port);if(stateChanged(before,after))return{changed:true,after};}catch{}await sleep(100);}try{after=await browserState(port);}catch{}return{changed:stateChanged(before,after),after};}
...
async function waitForNavigation(port,{before={},expectedUrl='',timeoutMs=7000,requireNewDocument=false}={}){
  const deadline=Date.now()+timeoutMs;let after={};
  while(Date.now()<deadline){
    try{
      after=await browserState(port);
      const ready=after.readyState==='interactive'||after.readyState==='complete';
      const urlOk=!expectedUrl||after.url===expectedUrl||after.url.startsWith(new URL(expectedUrl).origin);
...
export async function browserOpen(cfg,rawUrl){
  const url=normalizeHttpUrl(rawUrl);const port=await ensureBrowser(cfg);const target=await pageTarget(port);const before=await browserState(port).catch(()=>({url:target.url,title:target.title,timeOrigin:0}));invalidateSnapshot(target.id);
  await cdpCall(port,'Page.enable').catch(()=>{});const result=await cdpCall(port,'Page.navigate',{url:url.toString()});
  const after=await waitForNavigation(port,{before,expectedUrl:url.toString(),timeoutMs:8000});await activateTarget(port,target.id).catch(()=>{});
  const verified=Boolean(after.url&&(after.url===url.toString()||after.url.startsWith(url.origin)));if(!verified)throw new Error(`PC_BROWSER_NAVIGATION_NOT_VERIFIED:${after.url||'unknown'}`);
...
export async function browserTabs(cfg){const port=await ensureBrowser(cfg);const pages=await pageTargets(port);if(!pages.length)throw new Error('PC_BROWSER_NO_PAGE');const persisted=selectedTargetId||readPersistedSelection(port);if(!persisted||!pages.some(page=>page.id===persisted))setSelectedTarget(port,(pages.find(usefulPage)||pages[0]).id);else selectedTargetId=persisted;return{selectedTargetId,tabs:pages.slice(0,24).map((page,index)=>({index,id:page.id,title:String(page.title||'').slice(0,240),url:String(page.url||'').slice(0,1000),selected:page.id===selectedTargetId}))};}
export async function browserSelectTab(cfg,index){const port=await ensureBrowser(cfg);const pages=await pageTargets(port);const i=Math.max(0,Math.floor(Number(index)||0));const target=pages[i];if(!target)throw new Error('PC_BROWSER_TAB_NOT_FOUND');await activateTarget(port,target.id);const afterPages=await pageTargets(port);const selected=afterPages.find(page=>page.id===target.id);if(!selected)throw new Error('PC_BROWSER_TAB_SELECTION_NOT_VERIFIED');return{selected:true,verified:selectedTargetId===target.id,index:i,id:target.id,title:String(selected.title||'').slice(0,240),url:String(selected.url||'').slice(0,1000)};}
...
export async function browserSnapshot(cfg){
  ...
  const data=parseEval(await evaluate(port,expression))||{};snapshots.set(target.id,{token,url:data.url,elements:Array.isArray(data.elements)?data.elements:[],createdAt:Date.now()});return{...data,tabId:target.id,snapshotId:token};
}
async function snapshotElement(port,index){
  const target=await pageTarget(port);const snapshot=snapshots.get(target.id);if(!snapshot)throw new Error('PC_BROWSER_SNAPSHOT_REQUIRED');const current=await browserState(port);if(current.url!==snapshot.url){invalidateSnapshot(target.id);throw new Error('PC_BROWSER_STALE_SNAPSHOT');}
  ...
  const meta=parseEval(await evaluate(port,expression));if(!meta?.found){invalidateSnapshot(target.id);throw new Error('PC_BROWSER_STALE_SNAPSHOT');}return{target,snapshot,index:i,saved,meta};
}
export async function browserClick(cfg,index){
  ...
  const before=await browserState(port);const ref=JSON.stringify(saved.ref);const clicked=parseEval(await evaluate(port,`(() => {const ref=${ref};const el=[...document.querySelectorAll('[data-sexta-ref]')].find(node=>node.getAttribute('data-sexta-ref')===ref);if(!el)return false;el.scrollIntoView({block:'center',inline:'center'});el.focus();el.click();return true;})()`));invalidateSnapshot(target.id);if(!clicked)throw new Error('PC_BROWSER_CLICK_FAILED');const observed=await waitForStateChange(port,before,2800);return{clicked:true,index:i,element:meta,verified:observed.changed,before,after:observed.after,verification:observed.changed?'state_changed':'action_dispatched_no_observable_change',snapshotInvalidated:true};
}
...
async function navigationHistory(port){return cdpCall(port,'Page.getNavigationHistory');}
async function navigateHistory(cfg,delta){
  const port=await ensureBrowser(cfg);const target=await pageTarget(port);const before=await browserState(port);const history=await navigationHistory(port);const entries=Array.isArray(history.entries)?history.entries:[];const nextIndex=Number(history.currentIndex)+delta;const entry=entries[nextIndex];if(!entry)throw new Error(delta<0?'PC_BROWSER_NO_BACK_HISTORY':'PC_BROWSER_NO_FORWARD_HISTORY');invalidateSnapshot(target.id);await cdpCall(port,'Page.navigateToHistoryEntry',{entryId:entry.id});
  const after=await waitForNavigation(port,{before,expectedUrl:entry.url,timeoutMs:7000});const verified=after.url===entry.url&&after.url!==before.url;if(!verified)throw new Error('PC_BROWSER_HISTORY_NAVIGATION_NOT_VERIFIED');return{verified,before,after,entry:{id:entry.id,url:entry.url,title:entry.title||''},snapshotInvalidated:true};
}
...
export async function browserReload(cfg){
  const port=await ensureBrowser(cfg);const target=await pageTarget(port);const before=await browserState(port);invalidateSnapshot(target.id);await cdpCall(port,'Page.reload',{ignoreCache:false});const after=await waitForNavigation(port,{before,expectedUrl:before.url,timeoutMs:8000,requireNewDocument:true});const verified=after.url===before.url&&after.timeOrigin!==before.timeOrigin;if(!verified)throw new Error('PC_BROWSER_RELOAD_NOT_VERIFIED');return{reloaded:true,verified,before,after,snapshotInvalidated:true};
}
