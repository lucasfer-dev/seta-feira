...
function runPowerShell(script,timeout=8000,data={}){
  ...
  return new Promise((resolve,reject)=>{const child=spawn('powershell.exe',['-NoLogo','-NoProfile','-NonInteractive','-Command','-'],{windowsHide:true,stdio:['pipe','pipe','pipe'],env});let out='',err='',settled=false;
    ...
    child.stdout?.on('data',d=>{out+=d;});child.stderr?.on('data',d=>{err+=d;});child.on('error',error=>finish(reject,error));child.on('close',code=>code!==0?finish(reject,new Error(String(err||out||`powershell exit ${code}`).trim())):finish(resolve,String(out||'').trim()));child.stdin?.on('error',error=>{if(error?.code!=='EPIPE')finish(reject,error);});child.stdin?.end(`${String(script||'')}\r\n`,'utf8');
  });
}
...
function Get-SextaElements(){
  try{return $root.FindAll([System.Windows.Automation.TreeScope]::Subtree,[System.Windows.Automation.Condition]::TrueCondition)}catch{throw ('PC_UI_ENUMERATION_FAILED: '+$_.Exception.Message)}
}
`;
...
$fg=[SextaWin32]::GetForegroundWindow().ToInt64();$items=Get-Process|Where-Object{$_.MainWindowHandle -ne 0 -and $_.MainWindowTitle}|Select-Object -First ${max} @{N='process';E={$_.ProcessName}},@{N='pid';E={$_.Id}},@{N='title';E={$_.MainWindowTitle}},@{N='hwnd';E={$_.MainWindowHandle.ToInt64()}},@{N='active';E={$_.MainWindowHandle.ToInt64() -eq $fg}};@($items)|ConvertTo-Json -Compress`;
  const result=parseJson(await runPowerShell(script),[]);const windows=Array.isArray(result)?result:result?[result]:[];return{windows,count:windows.length,active:windows.find(item=>item.active)||null};
}
...
[pscustomobject]@{focused=$true;verified=$true;restored=[bool]$wasMinimized;process=$p.ProcessName;pid=$p.Id;title=$p.MainWindowTitle;hwnd=$h.ToInt64();beforeHwnd=$before;afterHwnd=$after;ambiguousMatches=$ranked.Count}|ConvertTo-Json -Compress`;
...
for($i=0;$i -lt $limit;$i++){try{$node=$elements.Item($i);$pwd=[bool]$node.Current.IsPassword;$r=$node.Current.BoundingRectangle;$name=if($pwd){'[password]'}else{[string]$node.Current.Name};[void]$items.Add([pscustomobject]@{index=$items.Count;depth=0;name=$name;automationId=[string]$node.Current.AutomationId;controlType=[string]$node.Current.ControlType.ProgrammaticName;enabled=[bool]$node.Current.IsEnabled;focused=[bool]$node.Current.HasKeyboardFocus;password=$pwd;x=[int]$r.X;y=[int]$r.Y;width=[int]$r.Width;height=[int]$r.Height})}catch{}}
[pscustomobject]@{window=[string]$root.Current.Name;hwnd=$handle.ToInt64();provider='findall-subtree';nodes=@($items);count=$items.Count;enumerated=$elements.Count}|ConvertTo-Json -Depth 5 -Compress`;
  return parseJson(await runPowerShell(script,12000),{});
}
...
function matchScript(kind){return String.raw`
$elements=Get-SextaElements;$matches=New-Object System.Collections.ArrayList
for($i=0;$i -lt $elements.Count -and $i -lt 1200;$i++){try{$node=$elements.Item($i);if($node.Current.IsPassword -or -not $node.Current.IsEnabled){continue};$name=([string]$node.Current.Name).Trim();$id=([string]$node.Current.AutomationId).Trim();$type=[string]$node.Current.ControlType.ProgrammaticName;$nl=$name.ToLowerInvariant();$il=$id.ToLowerInvariant();$score=99;if($nl -eq $needle -or $il -eq $needle){$score=0}elseif($nl.StartsWith($needle)-or$il.StartsWith($needle)){$score=1}elseif(($nl+' '+$il).Contains($needle)){$score=2};if($score -lt 99){$interactive=$type -match 'Button|Hyperlink|MenuItem|ListItem|TabItem|CheckBox|RadioButton|ComboBox|TreeItem|Edit|Document';[void]$matches.Add([pscustomobject]@{node=$node;score=$score;interactive=[bool]$interactive;type=$type})}}catch{}}
`;}
export async function uiClickText(text){const needle=String(text||'').trim().slice(0,240);if(!needle)throw new Error('PC_UI_TEXT_REQUIRED');if(SENSITIVE.test(needle))throw new Error('PC_UI_SENSITIVE_CONTROL_BLOCKED');const script=decodeHelper+uiPrelude+String.raw`$needle=(Get-SextaUtf8 'SEXTA_TEXT').Trim().ToLowerInvariant();`+matchScript('click')+String.raw`
...
export async function uiTypeText(text,target=''){const value=String(text||'').slice(0,4000);const needle=String(target||'').trim().slice(0,240);const script=decodeHelper+uiPrelude+String.raw`
$value=Get-SextaUtf8 'SEXTA_VALUE';$needle=(Get-SextaUtf8 'SEXTA_TARGET').Trim().ToLowerInvariant();$match=$null
if([string]::IsNullOrWhiteSpace($needle)){try{$focused=[System.Windows.Automation.AutomationElement]::FocusedElement;if($focused -and -not $focused.Current.IsPassword -and ([string]$focused.Current.ControlType.ProgrammaticName -match 'Edit|Document')){$match=$focused}}catch{}}
if($null -eq $match){$elements=Get-SextaElements;$matches=New-Object System.Collections.ArrayList;for($i=0;$i -lt $elements.Count -and $i -lt 1200;$i++){try{$node=$elements.Item($i);$type=[string]$node.Current.ControlType.ProgrammaticName;if(($type -eq 'ControlType.Edit'-or$type -eq 'ControlType.Document') -and $node.Current.IsEnabled){$name=([string]$node.Current.Name).Trim();$id=([string]$node.Current.AutomationId).Trim();$nl=$name.ToLowerInvariant();$il=$id.ToLowerInvariant();$score=if([string]::IsNullOrWhiteSpace($needle)){3}elseif($nl -eq $needle -or $il -eq $needle){0}elseif($nl.StartsWith($needle)-or$il.StartsWith($needle)){1}elseif(($nl+' '+$il).Contains($needle)){2}else{99};if($score -lt 99){[void]$matches.Add([pscustomobject]@{node=$node;score=$score})}}}catch{}};$chosen=$matches|Sort-Object score|Select-Object -First 1;if($chosen){$match=[System.Windows.Automation.AutomationElement]$chosen.node}}
if($null -eq $match){throw 'PC_UI_EDIT_NOT_FOUND'};if([bool]$match.Current.IsPassword){throw 'PC_UI_PASSWORD_FIELD_BLOCKED'};$typed=$false;$verified=$false;$via=''
try{$p=$match.GetCurrentPattern([System.Windows.Automation.ValuePatternIdentifiers]::Pattern);if($p -and -not $p.Current.IsReadOnly){$p.SetValue($value);$typed=$true;$via='value-pattern';Start-Sleep -Milliseconds 60;$verified=([string]$p.Current.Value -eq $value)}}catch{};if(-not $typed){try{$match.SetFocus();Start-Sleep -Milliseconds 80;$typed=[SextaWin32]::SendUnicode($value);$via='unicode-input';$verified=[bool]$typed}catch{}};if(-not $typed){throw 'PC_UI_TYPE_FAILED'}
