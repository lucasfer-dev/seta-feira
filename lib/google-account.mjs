...
export function isGoogleAccountQuestion(text = '') {
  ...
  const mentionsGoogle = /\b(?:google|gmail|e-?mail|conta)\b/.test(value);
  const asksIdentity = /\b(?:qual|que|quem|mostra|diz|fala|ver|veja)\b/.test(value)
    && /\b(?:email|e-?mail|gmail|conta)\b/.test(value)
    && /\b(?:logad[oa]|conectad[oa]|vinculad[oa]|autorizad[oa]|usando|ativa)\b/.test(value);
  return mentionsGoogle && asksIdentity;
}
