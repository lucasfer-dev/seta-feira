...
export const PC_DESKTOP_TOOL_DECLARATIONS = [
  ...
  { name: 'pc_window_focus', description: 'Traz para frente uma janela visível do Windows pelo título. Não executa ações dentro dela.', parameters: objectSchema({ title: stringProp('Parte distintiva do título da janela.'), handle: numberProp('Handle retornado por pc_window_list para desambiguar janelas.') }, ['title']) },
...
async function waitForCommand(commandId, timeoutMs = 12000) {
  ...
  while (Date.now() < deadline) {
    ...
    if (command.status === 'done') {
      ...
      if (result.ok === false || result.verified === false) return { state: 'failed', ok: false, verified: false, result, error: result.error || 'PC_ACTION_UNVERIFIED' };
      return { state: 'completed', ok: true, ...(typeof result.verified === 'boolean' ? { verified: result.verified } : {}), result };
    }
...
function commandFor(name, args = {}) {
  switch (name) {
    ...
    case 'pc_window_focus': return ['window_focus', { title: String(args.title || '').slice(0, 240), ...(args.handle != null ? { handle: args.handle } : {}) }];
    ...
    case 'pc_browser_select_tab': return ['browser_select_tab', { index: validatedIndex(args.index) }];
    case 'pc_browser_snapshot': return ['browser_snapshot', {}];
    case 'pc_browser_click': return ['browser_click', { index: validatedIndex(args.index) }];
    case 'pc_browser_type': return ['browser_type', { index: validatedIndex(args.index), text: String(args.text || '').slice(0, 4000) }];
...
export async function executePcDesktopTool(name, args = {}, options = {}) {
  ...
  const command = await queueCommand(pc.device_id, transport.action, transport.payload);
  const timeout = action === 'screen_analyze' ? 18000 : 35000;
...
function validatedIndex(value) {
  if (!Number.isInteger(value) || value < 0) throw new Error('PC_INVALID_INDEX');
  return value;
}
