...
export const PC_DESKTOP_TOOL_DECLARATIONS = [
  ...
  { name: 'pc_screen_analyze', description: 'Olha a tela do PC uma única vez usando captura local + visão multimodal. Use quando a árvore semântica não for suficiente ou o usuário pedir para olhar a tela.', parameters: objectSchema({ question: stringProp('O que deve ser observado na tela.'), scope: { type: 'string', enum: ['primary', 'all'], description: 'primary captura monitor principal; all captura a área virtual de monitores.' } }, ['question']) },
  ...
  { name: 'pc_ui_hotkey', description: 'Executa um atalho de teclado limitado a uma allowlist segura no Windows, como Ctrl+F, Ctrl+L, Ctrl+C, Ctrl+V, Ctrl+Tab, Alt+Left ou Escape.', parameters: objectSchema({ shortcut: stringProp('Atalho permitido em formato natural, por exemplo ctrl+f ou esc.') }, ['shortcut']) },
  { name: 'pc_browser_open', description: 'Abre/navega uma página no Browser Agent dedicado da SEXTA usando Chrome/Edge DevTools Protocol. Usa um perfil de automação separado e persistente.', parameters: objectSchema({ url: stringProp('URL http/https.') }, ['url']) },
  { name: 'pc_browser_snapshot', description: 'Lê o DOM visível da aba ativa do Browser Agent e retorna título, URL, texto e elementos interativos numerados. Prefira isto a visão por pixels em sites.', parameters: objectSchema({}) },
  { name: 'pc_browser_click', description: 'Clica em um elemento interativo numerado do último estado atual do Browser Agent. Alvos sensíveis como pagar, comprar, enviar, excluir, publicar ou confirmar são bloqueados.', parameters: objectSchema({ index: numberProp('Índice do elemento retornado por pc_browser_snapshot.') }, ['index']) },
  ...
  { name: 'pc_browser_back', description: 'Volta uma página no Browser Agent dedicado.', parameters: objectSchema({}) },
  { name: 'pc_agent_task', description: 'Executa uma tarefa multi-etapas no PC usando o ciclo observar → agir → verificar. Prefere DOM no navegador, UI Automation no desktop e visão por screenshot apenas como fallback. Não pode finalizar compras, pagamentos, envios, exclusões ou outras ações sensíveis por automação genérica.', parameters: objectSchema({ goal: stringProp('Objetivo explícito pedido pelo usuário para o PC.'), maxSteps: numberProp('Máximo de passos, de 2 a 10.') }, ['goal']) }
];
...
async function waitForCommand(commandId, timeoutMs = 12000) {
  ...
  while (Date.now() < deadline) {
    ...
    if (command.status === 'failed') return { state: 'failed', ok: false, result: command.result || {}, error: command.result?.message || 'PC_ACTION_FAILED' };
    await sleep(240);
  }
...
export async function executePcDesktopTool(name, args = {}) {
