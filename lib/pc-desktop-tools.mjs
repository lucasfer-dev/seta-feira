...
function enrichDeclaration(tool) {
  ...
  if (tool.name === 'pc_ui_type_text') {
    return {
      ...tool,
      description: 'Preenche texto em campo editável da janela alvo. Prefere a rota dedicada de digitação; se ela falhar em um campo identificado, tenta automaticamente a ação semântica UIA type antes de devolver falha. Nunca escreve em senha. Use title/hwnd para evitar digitar na janela errada.',
      parameters
    };
  }
...
function failedResult(result) {
  return result?.ok === false || result?.state === 'failed';
}
...
function menuDispatch(result, name, args = {}) {
  ...
  if (name === 'pc_ui_click_text') {
    return { label: String(native?.target?.name || args.text || '').trim(), automationId: String(native?.target?.automationId || '').trim() };
  }
  const operation = String(args.action || '').toLowerCase();
  if (name === 'pc_ui_action' && (operation === 'click' || operation === 'invoke')) {
    return { label: String(native.name || args.name || '').trim(), automationId: String(native.automationId || args.automationId || '').trim() };
  }
  return null;
}
...
async function stabilizeMenuActivation(name, args, result, options) {
  ...
  try {
    ...
    return { ...result, menuActivation: { semanticDispatched: true, postObservationAvailable: false, physicalFallbackUsed: false, error: String(error?.message || error).slice(0, 240) } };
  }
  const visible = findVisibleMenuNode(observation, dispatch);
  if (!visible) {
    return {
      ...result,
      result: { ...(result.result || {}), verified: true, verification: 'menu_item_no_longer_visible_after_semantic_dispatch' },
      menuActivation: { semanticDispatched: true, postObservationAvailable: true, itemStillVisible: false, physicalFallbackUsed: false }
    };
  }
  ...
  if (!physicalNative?.clicked) {
    return {
      ...
      result: { ...(result.result || {}), verified: false, error: 'PC_MENUITEM_ACTIVATION_NOT_VERIFIED' },
      menuActivation: { semanticDispatched: true, postObservationAvailable: true, itemStillVisible: true, physicalFallbackUsed: true, physicalResult: physical }
    };
  }
  return {
    ...physical,
    ...
    result: { ...physicalNative, verified: true, requiresObservation: true, via: 'native-menu-physical-fallback', semanticVia: 'SelectionItem' },
    menuActivation: {
      semanticDispatched: true,
      postObservationAvailable: true,
      itemStillVisible: true,
      physicalFallbackUsed: true,
      target: { name: visible.name, automationId: visible.automationId, controlType: visible.controlType, x, y }
    }
  };
}

async function recoverTypeText(args, initial, options) {
  const target = String(args.target || '').trim().slice(0, 240);
  const text = String(args.text || '').slice(0, 4000);
  if (!failedResult(initial) || !target || !text) return initial;
  const originalError = String(initial?.error || initial?.result?.error || initial?.result?.message || '').slice(0, 500);
  if (/PASSWORD_FIELD_BLOCKED|SENSITIVE_CONTROL_BLOCKED/i.test(originalError)) return initial;

  const semantic = await legacy.executePcDesktopTool('pc_ui_action', {
    action: 'type',
    name: target,
    controlType: 'Edit',
    value: text
  }, options);
  if (!failedResult(semantic)) {
    return {
      ...semantic,
      recovery: {
        from: 'pc_ui_type_text',
        via: 'pc_ui_action:type',
        originalError,
        verified: semantic?.ok === true && semantic?.state === 'completed'
      }
    };
  }
  return {
    ...
    recovery: {
      ...
      via: 'pc_ui_action:type',
      originalError,
      fallbackError: String(semantic?.error || semantic?.result?.error || semantic?.result?.message || '').slice(0, 500),
      verified: false
    }
  };
}

function recoverableUiError(result) {
...
export async function executePcDesktopTool(name, args = {}, options = {}) {
  ...
  let initial = await legacy.executePcDesktopTool(name, payload, options);
  if (name === 'pc_ui_type_text') initial = await recoverTypeText(payload, initial, options);
