...
import { createMission, updateMission } from './missions.mjs';
...
class SextaOrchestrator {
  ...
  routeMission(missionId) {
    ...
    const full = this.getMission?.(missionId);
    const requiredCapability = full?.requiredCapability || null;

    if (!requiredCapability) return updateMission(missionId, { status: 'running' });

    const device = chooseDevice(devices, requiredCapability);
    if (!device) return updateMission(missionId, { status: 'waiting_for_device' });
...
events.on('mission.completed', event => {
