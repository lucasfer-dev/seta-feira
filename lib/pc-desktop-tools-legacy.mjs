...
export const PC_DESKTOP_TOOL_DECLARATIONS = [
  ...
  { name: 'pc_window_list', description: 'Lista janelas reais do desktop Windows via HWND, incluindo processo, título, posição, tamanho e estado minimizado/maximizado.', parameters: objectSchema({ limit: numberProp('Máximo de janelas, entre 1 e 80.') }) },
  { name: 'pc_window_focus', description: 'Restaura uma janela minimizada e a traz realmente para frente. Pode localizar por título/processo ou usar o HWND retornado por pc_window_list. A ação só é sucesso quando o Windows confirma que ela virou a janela ativa.', parameters: objectSchema({ title: stringProp('Título ou nome do processo. Pode ser parcial.'), hwnd: numberProp('HWND exato retornado por pc_window_list, quando disponível.') }, ['title']) },
  { name: 'pc_window_close', description: 'Solicita o fechamento normal de uma janela de aplicativo que o usuário mandou fechar. Usa WM_CLOSE e verifica se a janela realmente fechou. Não força kill do processo; se o aplicativo abrir confirmação de salvar/descartar, a SEXTA deve observar e pedir confirmação antes de prosseguir.', parameters: objectSchema({ title: stringProp('Título ou nome do processo a fechar.'), hwnd: numberProp('HWND exato retornado por pc_window_list, quando disponível.') }, ['title']) },
  { name: 'pc_window_state', description: 'Minimiza, maximiza ou restaura uma janela e verifica o estado real depois da ação.', parameters: objectSchema({ title: stringProp('Título ou processo da janela.'), hwnd: numberProp('HWND exato opcional.'), state: { type: 'string', enum: ['restore', 'minimize', 'maximize'], description: 'Estado desejado.' } }, ['title', 'state']) },
  { name: 'pc_window_move_resize', description: 'Move e redimensiona uma janela normal para coordenadas e dimensões específicas do desktop, restaurando-a antes quando necessário e verificando os limites finais.', parameters: objectSchema({ title: stringProp('Título ou processo da janela.'), hwnd: numberProp('HWND exato opcional.'), x: numberProp('Coordenada X em pixels.'), y: numberProp('Coordenada Y em pixels.'), width: numberProp('Largura em pixels, mínimo 120.'), height: numberProp('Altura em pixels, mínimo 80.') }, ['title', 'x', 'y', 'width', 'height']) },
  ...
  { name: 'pc_ui_action', description: 'Executa uma ação semântica genérica em um controle da janela ativa. Localiza por Name, AutomationId e/ou ControlType e usa o padrão UI Automation correto; pode focar, invocar/clicar, selecionar, alternar, expandir/recolher, preencher ou rolar o item para a visão. Senhas e controles finais sensíveis são bloqueados.', parameters: objectSchema({ action: { type: 'string', enum: ['invoke', 'click', 'focus', 'select', 'toggle', 'expand', 'collapse', 'set_value', 'type', 'scroll_into_view'] }, name: stringProp('Name visível/acessível do controle.'), automationId: stringProp('AutomationId exato quando conhecido.'), controlType: stringProp('Tipo UIA, por exemplo Button, Edit, MenuItem, TabItem.'), value: stringProp('Valor/texto usado apenas em set_value ou type.') }, ['action']) },
  { name: 'pc_screen_click', description: 'Fallback para aplicativos cujo controle não é exposto por UI Automation. Clica numa coordenada identificada por uma observação de tela e exige também um rótulo semântico do alvo. Nunca deve ser a primeira opção e sempre exige uma nova observação para verificar o efeito.', parameters: objectSchema({ x: numberProp('Coordenada X absoluta no desktop.'), y: numberProp('Coordenada Y absoluta no desktop.'), label: stringProp('Rótulo do controle identificado visualmente; alvos sensíveis são bloqueados.') }, ['x', 'y', 'label']) },
...
function commandFor(name, args = {}) {
  switch (name) {
    ...
    case 'pc_window_list': return ['window_list', { limit: Math.max(1, Math.min(80, Number(args.limit) || 30)) }];
    case 'pc_window_focus': return ['window_focus', { title: String(args.title || '').slice(0, 240), hwnd: hwndArg(args.hwnd) }];
    case 'pc_window_close': return ['window_close', { title: String(args.title || '').slice(0, 240), hwnd: hwndArg(args.hwnd) }];
    case 'pc_window_state': return ['window_state', { title: String(args.title || '').slice(0, 240), hwnd: hwndArg(args.hwnd), state: ['restore', 'minimize', 'maximize'].includes(args.state) ? args.state : 'restore' }];
    case 'pc_window_move_resize': return ['window_move_resize', { title: String(args.title || '').slice(0, 240), hwnd: hwndArg(args.hwnd), x: Math.round(Number(args.x) || 0), y: Math.round(Number(args.y) || 0), width: Math.max(120, Math.round(Number(args.width) || 800)), height: Math.max(80, Math.round(Number(args.height) || 600)) }];
    ...
    case 'pc_ui_action': return ['ui_action', { action: String(args.action || 'invoke').slice(0, 40), name: String(args.name || '').slice(0, 240), automationId: String(args.automationId || '').slice(0, 240), controlType: String(args.controlType || '').slice(0, 120), value: String(args.value || '').slice(0, 4000) }];
    case 'pc_screen_click': return ['screen_click_point', { x: Math.round(Number(args.x) || 0), y: Math.round(Number(args.y) || 0), label: String(args.label || '').slice(0, 240) }];
...
export async function executePcDesktopTool(name, args = {}, options = {}) {
  ...
  const command = await queueCommand(pc.device_id, transport.action, transport.payload);
  const timeout = action === 'screen_analyze' ? 18000 : action === 'window_close' ? 12000 : 9000;
  ...
  const value = {
    ...final,
    handled: true,
    scope: action.startsWith('browser_') ? 'pc-browser' : action.startsWith('ui_') || action.startsWith('screen_') ? 'pc-hands' : action.startsWith('window_') ? 'pc-window' : 'pc',
    commandId: command.id,
    target: pc.name,
    action,
