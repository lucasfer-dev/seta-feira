...
export async function getLiveToolDeclarations() {
  ...
  const seen = new Set();
  return [...LIVE_TOOL_DECLARATIONS, ...ANDROID_HANDS_TOOL_DECLARATIONS, ...mcp].filter(tool => {
...
export async function executeTool(name, args = {}, options = {}) {
  ...
  const result = isAndroidHandsTool(name)
    ? await executeAndroidHandsTool(name, args, options)
    : await executeNativeTool(name, args, options);
...
export async function getToolCoreStatus() {
  ...
  return {
    ...
    nativeTools: LIVE_TOOL_DECLARATIONS.length + ANDROID_HANDS_TOOL_DECLARATIONS.length,
    handsTools: ANDROID_HANDS_TOOL_DECLARATIONS.length,
