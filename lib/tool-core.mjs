...
async function runPcAgentTask(args = {}, options = {}) {
  ...
  const system = [
    ...
    'AUTONOMIA LOCAL: este objetivo veio de uma ordem explícita do proprietário. Execute sem pedir confirmação intermediária todas as ações locais reversíveis necessárias: abrir/fechar/focar/mover janelas, navegar, clicar, selecionar, digitar, editar arquivos do workspace, usar atalhos, organizar apps e encadear múltiplas etapas.',
    'Ações irreversíveis ou de alto impacto (pagamento, compra, transferência financeira, exclusão permanente, instalação de software ou publicação externa) só podem ser finalizadas quando essa ação específica estiver claramente pedida no objetivo do usuário; caso contrário pare antes do controle final.',
    ...
    'Se surgir diálogo para salvar/descartar alterações, siga a escolha já expressa no objetivo. Se o usuário não tiver indicado a escolha e houver risco de perda de dados, pare antes de confirmar.',
