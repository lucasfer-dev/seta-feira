...
const TOOL_CORE_VERSION = '1.3.0-v4';
...
async function runPcAgentTask(args = {}, options = {}) {
  ...
  for (let round = 0; round < maxSteps + 3 && steps < maxSteps; round += 1) {
    ...
    if (!calls.length) {
      const verified = isPcTaskTraceVerified(trace);
      return {
        ok: verified,
        verified,
        handled: true,
        ...
        reason: verified ? undefined : 'PC_AGENT_TASK_UNVERIFIED',
        goal,
        steps,
        trace,
        summary: verified ? (parts.map(part => part.text || '').filter(Boolean).join('\n').trim() || 'Tarefa verificada.') : 'Não foi possível verificar a conclusão da tarefa; consulte as observações e falhas no trace.'
      };
    }
    ...
    for (const call of calls) {
      ...
      let result;
      try {
        if (!declarations.some(tool => tool.name === name)) throw new Error('PC_AGENT_TOOL_NOT_ALLOWED');
...
// A planner's final sentence is not evidence of completion.
export function isPcTaskTraceVerified(trace = []) {
  if (!trace.length) return false;
  const observations = new Set(['pc_window_list', 'pc_ui_tree', 'pc_browser_snapshot', 'pc_browser_tabs', 'pc_screen_analyze', 'pc_git_status', 'pc_system_info']);
  let outstanding = false;
  for (const entry of trace) {
    const result = entry.result;
    if (result?.ok !== true || result.state !== 'completed' || result.result?.ok === false || result.result?.verified === false) return false;
    if (observations.has(entry.tool)) outstanding = false;
    else outstanding = !(result.verified === true || result.result?.verified === true);
  }
  return !outstanding;
}
