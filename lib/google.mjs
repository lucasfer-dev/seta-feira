...
async function loadToken({ force = false } = {}) {
  if (!force && tokenLoadedAt && Date.now() - tokenLoadedAt < TOKEN_CACHE_MS) return tokenCache;
  if (!hasCloudStore()) {
  ...
  try {
    ...
    tokenCache = integration?.secret || null;
  } catch (error) {
    console.warn('[SEXTA Google] não consegui ler o token cloud:', error.message);
    tokenCache = null;
  }
  tokenLoadedAt = Date.now();
  return tokenCache;
...
async function saveToken(token) {
  tokenCache = token;
  tokenLoadedAt = Date.now();
  if (!hasCloudStore()) return token;
  await saveIntegration(GOOGLE_PROVIDER, token, {
...
export async function googleStatus() {
  ...
  return {
    ...
    scopes: GOOGLE_SCOPES,
    tokenStore: hasCloudStore() ? 'supabase-encrypted' : 'memory-only'
  };
}
...
export async function exchangeGoogleCode(code, state) {
  ...
  const previous = await loadToken({ force: true }) || {};
  await saveToken({ ...previous, ...data, expires_at: Date.now() + (Number(data.expires_in || 3600) * 1000) - 60_000 });
  return googleStatus();
}
