...
export async function queueCommand(targetDeviceId, action, payload = {}) {
  const allowed = new Set(['open_url','open_app','open_project','git_status','get_system_info','read_clipboard','copy_text','codex_task','browser_open','browser_tabs','browser_snapshot','screen_analyze']);
...
export async function getCommand(id) {
  if (modeInfo().cloud) {
    const rows = await sb('sexta_commands', { query: { select:'id,status,result,action,payload,target_device_id,updated_at', id:`eq.${id}`, limit:'1' } });
    return rows?.[0] || null;
  }
