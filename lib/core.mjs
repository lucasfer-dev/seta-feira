...
import { invalidateSyncCache } from './sync-cache.mjs';

const OWNER = 'owner';
const memoryState = {
  messages: [], memories: [], vaultNotes: [], devices: new Map(), commands: [], events: [], notifications: [], pushTokens: [], pendingActions: [], settings: {}
};
...
export async function createPendingAction({ toolName, args = {}, context = {}, summary = '', expiresAt }) {
  const now = new Date().toISOString();
  const row = {
    id: crypto.randomUUID(),
    owner_id: OWNER,
    tool_name: String(toolName || '').slice(0, 120),
    args: args && typeof args === 'object' ? args : {},
    context: context && typeof context === 'object' ? context : {},
    summary: String(summary || '').slice(0, 800),
    status: 'pending',
    result: null,
    expires_at: expiresAt || new Date(Date.now() + 10 * 60_000).toISOString(),
    created_at: now,
    updated_at: now
  };
  if (modeInfo().cloud) {
    const { id, ...insert } = row;
    const rows = await sb('sexta_pending_actions', { method: 'POST', body: insert, prefer: 'return=representation' });
    return rows?.[0] || row;
  }
  memoryState.pendingActions.unshift(row);
  memoryState.pendingActions = memoryState.pendingActions.slice(0, 50);
  return row;
}
...
export async function getPendingAction(id = '') {
  if (modeInfo().cloud) {
    const query = {
      select: 'id,tool_name,args,context,summary,status,result,expires_at,created_at,updated_at',
      owner_id: 'eq.owner',
      order: 'created_at.desc',
      limit: '1'
    };
    if (id) query.id = `eq.${id}`;
    else query.status = 'eq.pending';
    const rows = await sb('sexta_pending_actions', { query });
    return rows?.[0] || null;
  }
  if (id) return memoryState.pendingActions.find(item => item.id === id) || null;
  return memoryState.pendingActions.find(item => item.status === 'pending') || null;
}

export async function claimPendingAction(id = '') {
  ...
  if (new Date(action.expires_at).getTime() <= Date.now()) {
    await finishPendingAction(action.id, 'expired', { ok: false, error: 'CONFIRMATION_EXPIRED' });
    return { ...action, status: 'expired' };
  }
  const patch = { status: 'executing', updated_at: new Date().toISOString() };
  if (modeInfo().cloud) {
    const rows = await sb('sexta_pending_actions', {
      method: 'PATCH',
      query: { id: `eq.${action.id}`, status: 'eq.pending' },
      body: patch,
      prefer: 'return=representation'
    });
    return rows?.[0] ? { ...rows[0], _claimed: true } : await getPendingAction(action.id);
  }
  Object.assign(action, patch);
  return { ...action, _claimed: true };
}

export async function finishPendingAction(id, status, result = null) {
  const safeStatus = ['completed', 'failed', 'canceled', 'expired'].includes(status) ? status : 'failed';
  const patch = { status: safeStatus, result, updated_at: new Date().toISOString() };
  if (modeInfo().cloud) {
    const rows = await sb('sexta_pending_actions', { method: 'PATCH', query: { id: `eq.${id}` }, body: patch, prefer: 'return=representation' });
    return rows?.[0] || null;
  }
  const action = memoryState.pendingActions.find(item => item.id === id);
...
export function sanitizeSettings(input = {}) {
  const clamp = (v, d) => Math.max(0, Math.min(100, Number.isFinite(Number(v)) ? Number(v) : d));
  return {
    humor: clamp(input.humor, 68), sarcasm: clamp(input.sarcasm, 42),
    proactivity: clamp(input.proactivity, 55), verbosity: clamp(input.verbosity, 32),
...
function personalityPrompt(settings) {
  ...
  return `Você é ${s.name}, uma assistente pessoal cloud-first com personalidade consistente.\n` +
    `Fale em português brasileiro natural. Humor=${s.humor}/100; sarcasmo=${s.sarcasm}/100; proatividade=${s.proactivity}/100; verbosidade=${s.verbosity}/100.\n` +
    `Seu humor deve ser situacional e curto, nunca forçado. Você pode discordar com educação. Não aja como SAC, coach ou personagem exagerada.\n` +
    `Você não possui emoções ou consciência reais: a personalidade é um estilo de interação. Não invente que executou uma ação se o contexto não confirmar.\n` +
    `Para ações no computador, confie apenas no bloco AÇÃO DO SISTEMA. Se houver uma ação enfileirada, diga isso de forma natural.\n` +
    `Proteja privacidade: nunca peça senhas, tokens, cookies ou chaves privadas. Seja concisa em conversa casual e detalhada apenas quando necessário.`;
}
...
export async function answer({ message, conversationId, deviceId, settings, clientContext = {}, actionContext = null }) {
  ...
  const history = await getMessages(conversationId, complex ? 15 : 8);
...
function demoAnswer(message, actionContext, memories, settings) {
  ...
  if (actionContext) {
    ...
    return `Certo. Já enviei para ${actionContext.target} a ordem de ${names[actionContext.action] || 'executar a ação'}. ${s.sarcasm > 55 ? 'Agora dependemos do computador colaborar, o que já é pedir bastante.' : ''}`.trim();
  }
  if (/^(oi|ol[aá]|e a[ií]|sexta\??)$/i.test(String(message).trim())) return 'Estou aqui. O que você aprontou?';
  if (t.includes('quem é você') || t.includes('o que você é')) return 'Sou a Sexta: a mesma assistente no celular, no PC e no navegador. Neste momento meu cérebro de demonstração está ativo; com a chave do Gemini eu fico consideravelmente menos decorativa.';
  if (t.includes('lembra') && memories.length) return `Lembro de algumas coisas. A mais forte agora é: “${memories[0].content}”. Milagre tecnológico: memória sem post-it.`;
  if (t.includes('hora')) return `Agora são ${new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}. E sim, eu sei que você poderia ter olhado no canto da tela.`;
