...
import { runEventEngine } from './event-engine.mjs';
...
export async function runMonitor() {
  const results={ gmail:null, eventEngine:null };
  try { results.gmail=await ingestGmailUnread(); } catch (error) { results.gmail={ok:false,error:error.message}; }
  try { results.eventEngine=await runEventEngine(); } catch (error) { results.eventEngine={ok:false,error:error.message}; }
