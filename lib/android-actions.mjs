...
export function detectAndroidIntent(text = '') {
  ...
  let m = raw.match(/\b(?:responde|responder|responda)\s+(?:no\s+whatsapp\s+)?(?:a|ao|o|pra|para)?\s*([^,.:]+?)\s+(?:no\s+whatsapp\s+)?(?:dizendo|falando|com|que)\s+(.+)$/i);
  if (m?.[1] && m?.[2]) {
    return { action:'notification_reply', payload:{ package:'whatsapp', recipient:m[1].trim(), text:m[2].trim() }, reply:`Vou responder ${m[1].trim()} pelo WhatsApp.` };
  }
  m = raw.match(/\b(?:responde|responder|responda)\s+(?:a\s+)?(?:ultima|última)\s+(?:mensagem|notificacao|notificação)(?:\s+do\s+whatsapp)?\s+(?:dizendo|falando|com|que)\s+(.+)$/i);
  if (m?.[1]) return { action:'notification_reply', payload:{ package:'whatsapp', text:m[1].trim() }, reply:'Vou responder a última conversa respondível do WhatsApp.' };
