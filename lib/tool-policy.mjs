...
const SCREEN_READ_PATTERN = /(?:\b(?:olha|olhe|ver|veja|vê|leia|ler|mostra|mostre|identifica|identifique|tela|screen|interface|janela|window|site|p[aá]gina|browser|navegador)\b|o\s+que\s+(?:tem|aparece|esta|está))/i;
...
const PC_PATTERN = /\b(?:pc|computador|windows|desktop|notebook|tela|navegador|browser|janela|programa|aplicativo)\b/i;
...
function explicitForTool(name, userText = '') {
  ...
  if (name === 'android_ui_snapshot' || ['pc_ui_tree', 'pc_screen_analyze', 'pc_window_list', 'pc_browser_snapshot'].includes(name)) {
