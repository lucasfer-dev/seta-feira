...
const EXPLICIT_ACTION_PATTERN = /\b(?:manda|mande|mandar|envia|envie|enviar|responde|responda|responder|cria|crie|criar|marca|marque|marcar|agenda|agende|agendar|adiciona|adicione|adicionar|salva|salve|salvar|abre|abra|abrir|liga|ligue|ligar|desliga|desligue|desligar|muda|mude|alterar|altera|altere|executa|execute|executar|roda|rode|rodar|faz|faça|fazer|corrige|corrija|corrigir|edita|edite|editar|implementa|implemente|implementar|procura|procure|procurar|pesquisa|pesquise|pesquisar|acha|ache|encontra|encontre|vai|ir|navega|navegue|navegar|seleciona|selecione|selecionar|toca|toque|tocar|clica|clique|clicar|rola|role|rolar|volta|volte|voltar|send|reply|create|schedule|open|turn on|turn off|run|execute|edit|implement|search|find|navigate|select|tap|click|scroll|back)\b/i;
...
const SCREEN_READ_PATTERN = /(?:\b(?:olha|olhe|ver|veja|vê|leia|ler|mostra|mostre|identifica|identifique|tela|screen|interface)\b|o\s+que\s+(?:tem|aparece|esta|está))/i;
...
function explicitForTool(name, userText = '') {
  ...
  if (name === 'android_ui_snapshot') return SCREEN_READ_PATTERN.test(text) || EXPLICIT_ACTION_PATTERN.test(text);
...
export function getToolPolicy(name, args = {}, external = null) {
  ...
  if (name === 'android_ui_snapshot') {
    return {
      version: POLICY_VERSION,
      risk: 'low',
      sideEffect: 'screen-read',
      requiresExplicit: true,
      destructive: false,
      external: false
    };
