...
function explicitForTool(name, userText = '', args = {}) {
  ...
  if (name === 'google_send_email' || name === 'whatsapp_send_message' || name === 'android_reply_notification') {
    return COMMUNICATION_PATTERN.test(text) || Boolean(args?.recipient || args?.text || args?.body);
  }
...
export function evaluateToolPolicy(name, args = {}, context = {}) {
  ...
  if (enforceExplicit && policy.requiresExplicit && !explicitForTool(name, userText, args)) {
