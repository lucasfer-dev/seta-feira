...
const AI_AUTO_MEMORY_ENABLED = /^(?:1|true|yes|on)$/i.test(String(process.env.SEXTA_AUTO_MEMORY_AI || ''));
...
async function askMemoryModel({ userText, assistantText, existing }) {
  const c = config();
  if (!c.geminiKey || !AI_AUTO_MEMORY_ENABLED) return { memories: [] };
...
export async function absorbAutomaticMemory({ userText = '', assistantText = '', source = 'auto-memory' } = {}) {
  ...
  if (cleanUser.length < 5 || SENSITIVE.test(cleanUser)) return { saved: [], deleted: [], skipped: true };

  if (!AI_AUTO_MEMORY_ENABLED) {
    return { saved: [], deleted: [], skipped: true, reason: 'auto_memory_ai_disabled' };
  }
  ...
  try { proposal = await askMemoryModel({ userText: cleanUser, assistantText, existing }); }
    ...
    return { saved: [], deleted: [], error: error.message };
  }
