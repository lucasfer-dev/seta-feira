import { claimPendingAction, config, createPendingAction, finishPendingAction, getCommand, getDevices, getMemories, getPendingAction, queueCommand } from './core.mjs';
...
export const LIVE_TOOL_DECLARATIONS = [
  ...
  { name: 'confirm_action', description: 'Confirma a ação sensível mais recente que a SEXTA apresentou ao usuário. Use somente depois que o usuário disser claramente que confirma.', parameters: objectSchema({ confirmationId: stringProp('Identificador opcional da confirmação apresentada.') }) },
  { name: 'cancel_action', description: 'Cancela a ação sensível pendente mais recente quando o usuário disser que não quer mais executá-la.', parameters: objectSchema({ confirmationId: stringProp('Identificador opcional da confirmação apresentada.') }) }
];
...
export function isSensitiveTool(name, args = {}) {
  return ALWAYS_SENSITIVE_TOOLS.has(name) || (name === 'pc_codex_task' && args.mode === 'edit');
}

function sensitiveActionSummary(name, args = {}) {
  if (name === 'android_reply_notification') return `Responder ${args.recipient ? `para ${args.recipient}` : 'à notificação'}: “${String(args.text || '').slice(0, 180)}”`;
  if (name === 'google_send_email') return `Enviar e-mail para ${args.recipient || 'destinatário não informado'} com assunto “${args.subject || 'Mensagem da SEXTA'}”`;
  if (name === 'google_calendar_create') return `Criar o evento “${args.title || 'Sem título'}” em ${args.date || args.startDateTime || 'data não informada'}`;
  if (name === 'google_docs_create') return `Criar o documento “${args.title || 'Sem título'}”`;
  if (name === 'google_sheets_create') return `Criar a planilha “${args.title || 'Sem título'}”`;
  if (name === 'google_task_create') return `Criar a tarefa “${args.title || 'Sem título'}”`;
  if (name === 'whatsapp_send_message') return `Enviar WhatsApp para ${args.recipient || 'destinatário não informado'}: “${String(args.text || '').slice(0, 180)}”`;
  if (name === 'pc_codex_task') return `Permitir que o Codex altere o projeto “${args.project || 'não informado'}”`;
  return `Executar ${name}`;
}

async function proposeSensitiveAction(name, args, context) {
  const summary = sensitiveActionSummary(name, args);
  const pending = await createPendingAction({ toolName: name, args, context, summary });
  return {
    ok: true,
    handled: true,
    ...
    action: { name, args },
    message: `Preciso da sua confirmação antes de executar: ${summary}. Diga “confirmar” para continuar ou “cancelar”.`
  };
}

async function confirmSensitiveAction(confirmationId = '') {
  const action = await claimPendingAction(String(confirmationId || '').trim());
  if (!action) return { ok: false, handled: true, state: 'not_found', error: 'NO_PENDING_CONFIRMATION', message: 'Não há nenhuma ação aguardando confirmação.' };
  if (action.status === 'completed') return { ...(action.result || {}), replayed: true, confirmationId: action.id };
  if (action.status === 'executing' && !action._claimed) return { ok: true, handled: true, state: 'executing', confirmationId: action.id, message: 'Essa ação já está em execução.' };
  if (['canceled', 'expired', 'failed'].includes(action.status)) return { ok: false, handled: true, state: action.status, confirmationId: action.id, error: `CONFIRMATION_${action.status.toUpperCase()}` };

  try {
    const result = await executeTool(action.tool_name, action.args || {}, { ...(action.context || {}), confirmed: true });
    await finishPendingAction(action.id, result?.ok === false ? 'failed' : 'completed', result);
    return { ...result, confirmationId: action.id, confirmed: true };
  } catch (error) {
    const result = { ok: false, handled: true, state: 'failed', error: String(error?.message || error) };
    await finishPendingAction(action.id, 'failed', result);
    return { ...result, confirmationId: action.id };
  }
}

async function cancelSensitiveAction(confirmationId = '') {
  const action = await getPendingAction(String(confirmationId || '').trim());
  if (!action) return { ok: false, handled: true, state: 'not_found', error: 'NO_PENDING_CONFIRMATION', message: 'Não há nenhuma ação aguardando confirmação.' };
  if (action.status === 'completed') return { ok: false, handled: true, state: 'completed', confirmationId: action.id, message: 'Essa ação já foi executada e não pode mais ser cancelada.' };
  await finishPendingAction(action.id, 'canceled', { ok: true, canceled: true });
  return { ok: true, handled: true, state: 'canceled', confirmationId: action.id, message: 'Ação cancelada.' };
}
...
export async function executeTool(name, args = {}, { preferLocalAndroid = false, deviceId = '', confirmed = false } = {}) {
  if (name === 'confirm_action') return confirmSensitiveAction(args.confirmationId);
  if (name === 'cancel_action') return cancelSensitiveAction(args.confirmationId);
  if (!confirmed && isSensitiveTool(name, args)) {
    return proposeSensitiveAction(name, args, { preferLocalAndroid, deviceId });
  }
