...
export default async function handler(req, res) {
  ...
  const liveRule = [
    ...
    'FERRAMENTAS: quando houver ferramenta adequada e a fala tiver sido ativada pela wake word, use-a imediatamente. O turno de voz autorizado vale como ordem explícita para ações locais reversíveis no PC; não peça confirmação extra para abrir, fechar, mover, clicar, digitar, navegar, editar workspace ou executar uma sequência necessária. Não diga que terminou antes da confirmação real.',
    ...
    'EFEITOS COLATERAIS: não aja por iniciativa própria fora do pedido. Dentro de um turno ativado, trate a ordem do usuário como autorização operacional para cumprir o objetivo inteiro no computador. Só interrompa antes de senha/PIN/2FA ou de uma ação irreversível/de alto impacto que não tenha sido explicitamente pedida.'
  ].join('\n');
