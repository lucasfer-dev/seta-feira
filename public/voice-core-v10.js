...
(() => {
  ...
  function rememberPreRoll(frame) {
    ...
    let totalMs = preRollFrames.reduce((sum, item) => sum + item.length / INPUT_RATE * 1000, 0);
    while (preRollFrames.length > 1 && totalMs > PRE_ROLL_MS) {
      const removed = preRollFrames.shift();
      totalMs -= removed.length / INPUT_RATE * 1000;
    }
  }
  ...
  function beginLocalSpeech(now) {
    ...
    sendRealtime({ activityStart:{} });
    for (const buffered of preRollFrames) sendAudioFrame(buffered);
    preRollFrames = [];
  ...
  function processMicFrame(raw) {
    ...
    if (activityOpen) sendAudioFrame(frame);
    else rememberPreRoll(frame);
  ...
  async function handleToolCall(toolCall) {
    ...
    reportMetric('tool_batch_complete', {
      count:responses.length,
      failed:responses.filter(item => item?.response?.ok === false).length
    });
  ...
  async function connectLive(reason = 'initial') {
    ...
    try {
      ...
      socket.onmessage = event => {
        void handleServerMessage(event, socket).catch(error => {
          console.error('[SEXTA v10] mensagem:', error);
          try { socket.close(4002, 'message-handler-failed'); } catch {}
        });
      };
      socket.onerror = event => console.warn('[SEXTA v10] websocket:', event);
      ...
      connectingSocket = null; console.error('[SEXTA v10] conexão:', error);
