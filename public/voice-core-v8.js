...
(() => {
  ...
  function reportMetric(kind, extra = {}) {
    void api('/api/live-metrics', {
      method:'POST',
      body:JSON.stringify({ kind:`voice_core_v8:${kind}`, platform:ORIGIN, state, ...extra })
    }).catch(() => {});
  }
  ...
  function speechThreshold() {
    const base = IS_ANDROID ? 0.012 : 0.0068;
    const adaptive = noiseFloor * (assistantSpeaking ? 4.8 : 3.0);
    return Math.max(base, assistantSpeaking ? 0.022 : 0, adaptive);
  }
  ...
  function beginActivity(now) {
    if (activityOpen || !sessionActive || !setupComplete) return;
    if (assistantSpeaking) stopOutput(false);

    serverTurnComplete = false;
    settleGeneration += 1;
    turn = freshTurn();
    turn.localSpeechStartAt = now;
    emitTranscript();

    if (!sendRealtime({ activityStart:{} })) return;
    activityOpen = true;
    transition('user_speaking');
    reportMetric('activity_start', { noiseFloor:Number(noiseFloor.toFixed(5)), threshold:Number(speechThreshold().toFixed(5)) });

    for (const frame of preRollFrames) sendAudioFrame(frame);
    preRollFrames = [];
  }

  function endActivity(now) {
    if (!activityOpen) return;
    if (!sendRealtime({ activityEnd:{} })) return;
    activityOpen = false;
    speechEvidenceMs = 0;
    turn.activityEndAt = now;
    transition('thinking');
    reportMetric('activity_end', {
      speechMs: turn.localSpeechStartAt ? Math.round(now - turn.localSpeechStartAt) : null
    });
  }
  ...
  function processMicFrame(raw) {
    ...
    if (!activityOpen && !assistantSpeaking && level < 0.018) {
    ...
    if (!activityOpen) {
      ...
      if (voiced) {
        ...
        lastSpeechAt = now;
        if (speechEvidenceMs >= START_CONFIRM_MS) beginActivity(now - speechEvidenceMs);
      } else {
        speechEvidenceMs = Math.max(0, speechEvidenceMs - frameMs * 0.5);
      }
      return;
    }
    ...
    if (lastSpeechAt && now - lastSpeechAt >= END_SILENCE_MS) {
      endActivity(now);
      preRollFrames = [];
    }
  }
  ...
  async function settleCompletedTurn() {
    ...
    while (sessionActive && generation === settleGeneration && performance.now() - started < 30000) {
      if (serverTurnComplete && pendingToolCalls === 0 && playbackDrained()) {
        await new Promise(resolve => setTimeout(resolve, OUTPUT_SETTLE_MS));
        if (generation !== settleGeneration || !sessionActive) return;
        assistantSpeaking = false;
        const snapshot = { ...turn };
        serverTurnComplete = false;
        turn = freshTurn();
        ...
        reportMetric('turn', {
          ...
          speechEndToSpeakingMs: snapshot.firstAudioAt && snapshot.activityEndAt ? Math.round(snapshot.firstAudioAt - snapshot.activityEndAt) : null,
          speechEndToFinalMs: snapshot.firstFinalAt && snapshot.activityEndAt ? Math.round(snapshot.firstFinalAt - snapshot.activityEndAt) : null
        });
        if (!activityOpen && pendingToolCalls === 0) transition('listening');
        return;
      }
  ...
  async function startMicrophone() {
    ...
    reportMetric('capture_ready', {
      ...
      audioSource:`manual-vad:${inputContext.sampleRate}->${INPUT_RATE}`
    });
    transition('listening');
  }
  ...
  function stopMicrophone() {
    ...
    try { if (activityOpen) sendRealtime({ activityEnd:{} }); } catch {}
    activityOpen = false;
  ...
  async function buildSystemInstruction() {
    ...
    return [
      ...
      'Responda assim que a intenção estiver clara. Respeite hesitações naturais, mas não fique esperando indefinidamente.',
    ...
    try {
      await api('/api/live-turn', {
        ...
        body:JSON.stringify({
          ...
          deviceId:localStorage.getItem('sexta_device_id') || 'voice-v8',
          userText:user,
  ...
  async function executeLiveTool(call) {
    ...
    const deviceId = localStorage.getItem('sexta_device_id') || (IS_ANDROID ? 'android-native' : 'voice-v8');
  ...
  async function handleToolCall(toolCall) {
    ...
    const responses = await Promise.all(calls.map(async call => {
      try {
        ...
        return {
          id:call.id,
          name:call.name,
          response:result ?? { ok:true, state:'completed' },
    ...
    else if (!assistantSpeaking && !activityOpen && pendingToolCalls === 0) transition('listening');
  }
  ...
  async function handleServerMessage(event, socket) {
    ...
    if (message.setupComplete) {
      ...
      try { await startMicrophone(); }
        ...
        console.error('[SEXTA v8] microfone:', error);
    ...
    if (content.waitingForInput) {
      assistantSpeaking = false;
      if (!activityOpen && pendingToolCalls === 0) transition('listening', { label:'Ouvindo.' });
    }

    if (content.interrupted) {
      stopOutput(false);
      if (activityOpen) transition('user_speaking');
      else transition('listening');
    }
  ...
  async function connectLive(reason = 'initial') {
    ...
    try {
      ...
      if (session.vadMode !== 'manual') throw new Error('servidor não ativou VAD manual');
      ...
      socket.onerror = event => console.warn('[SEXTA v8] websocket:', event);
      ...
      connectingSocket = null;
      console.error('[SEXTA v8] conexão:', error);
